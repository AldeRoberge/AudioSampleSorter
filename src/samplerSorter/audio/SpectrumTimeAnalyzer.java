...
public class SpectrumTimeAnalyzer extends JPanel implements KJDigitalSignalProcessor {
	...
	private static final int DEFAULT_SPECTRUM_ANALYSER_BAND_COUNT = 19;
	...
	private static final float MAX_SPECTRUM_ANALYSER_DECAY = 0.08f;
	private static final Color DEFAULT_BACKGROUND_COLOR = new Color(0, 0, 128);
	private static final Color DEFAULT_SCOPE_COLOR = new Color(255, 192, 0);
	...
	public void setVisColor(String viscolor) {
		...
		try {
			...
			try {
				...
				Logger.logError("SpectrumTimeAnalyzer", "Error", e);
			}
		}
	}
	 ...
	 * @return
	 */
	Color getColor(String linecolor) {
	...
	private void drawVUMeter(Graphics pGrp, float[] pLeft, float[] pRight, float pFrrh) {
		...
		float wLeft = 0.0f;
		float wRight = 0.0f;
		float vuDecay = DEFAULT_VU_METER_DECAY;
	...
	private static Color[] getDefaultSpectrumAnalyserColors() {
	 ...
	 */
	synchronized void setSpectrumAnalyserColors(Color[] pColors) {
